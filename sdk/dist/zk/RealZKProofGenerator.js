...
exports.RealZKProofGenerator = void 0;
...
component main = StakeProof();
     ...
     */
    async generateRealVerificationKey() {
     ...
     */
    async generateRealZKProof(privateInputs, publicInputs) {
    ...
    async poseidonHash(inputs) {
        ...
        const poseidon = await Promise.resolve().then(() => __importStar(require('circomlib'))).then(lib => lib.poseidon);
    ...
    async generateWitness(inputs) {
        ...
        const merkleValid = this.verifyMerkleConstraint(inputs.actualAmount, inputs.merklePathElement, inputs.merklePathIndex, inputs.merkleRoot);
        if (!merkleValid) {
     ...
     */
    verifyMerkleConstraint(leaf, pathElements, pathIndices, root) {
    ...
    hashPair(left, right) {
        ...
        return BigInt(ethers_1.ethers.utils.keccak256(ethers_1.ethers.utils.defaultAbiCoder.encode(['uint256', 'uint256'], combined)));
    }
    /**
     * Verify REAL ZK proof
     */
    async verifyRealZKProof(proof) {
     ...
     */
    async verifyPairing(a, b, c, publicSignals) {
...
exports.RealZKProofGenerator = RealZKProofGenerator;
//# sourceMappingURL=RealZKProofGenerator.js.map