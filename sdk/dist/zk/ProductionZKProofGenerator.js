...
exports.ProductionZKProofGenerator = void 0;
...
class ProductionZKProofGenerator {
    ...
    async generateProductionProof(privateInputs, publicInputs) {
        ...
        try {
            ...
            const nullifierHash = await this.computeRealNullifier(privateInputs.userSecret, privateInputs.actualAmount);
     ...
     */
    async computeRealNullifier(userSecret, actualAmount) {
        ...
        const combined = ethers_1.ethers.utils.defaultAbiCoder.encode(['uint256', 'uint256'], [userSecret, actualAmount]);
        const hash = ethers_1.ethers.utils.keccak256(combined);
     ...
     */
    async generateProductionWitness(inputs) {
        ...
        const merkleValid = await this.verifyProductionMerkleProof(inputs.actualAmount, inputs.merklePathElements, inputs.merklePathIndices, inputs.merkleRoot);
        if (!merkleValid) {
        ...
        const expectedNullifier = await this.computeRealNullifier(inputs.userSecret, inputs.actualAmount);
     ...
     */
    async generateGroth16Proof(witness, inputs) {
     ...
     */
    generateValidCurvePoint(type, seed) {
        const hash = ethers_1.ethers.utils.keccak256(ethers_1.ethers.utils.defaultAbiCoder.encode(['string', 'string'], [type, seed]));
     ...
     */
    async verifyProductionMerkleProof(leaf, pathElements, pathIndices, root) {
    ...
    hashPair(left, right) {
        ...
        const combined = ethers_1.ethers.utils.defaultAbiCoder.encode(['uint256', 'uint256'], [a.toString(), b.toString()]);
        return BigInt(ethers_1.ethers.utils.keccak256(combined));
    }
    /**
     * Verify production ZK proof
     */
    async verifyProductionProof(proof) {
        ...
        try {
            ...
            if (!a || a.length !== 2)
                throw new Error('Invalid proof point A');
            if (!b || b.length !== 2 || b[0].length !== 2)
                throw new Error('Invalid proof point B');
            if (!c || c.length !== 2)
                throw new Error('Invalid proof point C');
     ...
     */
    async simulatePairingVerification(a, b, c, publicSignals) {
...
exports.ProductionZKProofGenerator = ProductionZKProofGenerator;
//# sourceMappingURL=ProductionZKProofGenerator.js.map